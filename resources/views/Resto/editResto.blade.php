                    ...
                    <div class="h-64 relative bg-surface-container-highest flex items-center justify-center overflow-hidden">
                        ...
                        @else
                            ...
                            <div class="absolute inset-0 flex flex-col items-center justify-center p-6 text-center">
                                ...
                                <p class="text-xs text-on-surface-variant">Koordinat peta belum diatur.</p>
                            </div>
                        @endif
                    </div>
...
<div class="fixed inset-0 z-50 flex items-center justify-center p-6 bg-black/60 backdrop-blur-md hidden" id="edit-profile-modal">
    <!-- Modal Card -->
    <form action="{{ route('resto.profile.update') }}" method="POST" enctype="multipart/form-data" class="bg-surface-container-high/90 w-full max-w-2xl max-h-[95vh] rounded-lg shadow-2xl flex flex-col overflow-hidden border border-white/5">
        @csrf
        <!-- Modal Header -->
        ...
        <div class="p-8 overflow-y-auto space-y-6">
            ...
            <div class="space-y-2">
                <label class="font-label-md text-label-md text-[#D9C5A0] block ml-1">Foto / Thumbnail Restoran</label>
                <div class="flex items-center gap-4">
                    <div class="w-24 h-24 rounded-lg overflow-hidden border border-white/10 bg-surface-container-lowest flex items-center justify-center shrink-0">
                        <img id="thumbnail-preview" src="{{ $restaurant->photo_url }}" alt="Preview" class="w-full h-full object-cover">
                    </div>
                    <div class="flex-1">
                        <input type="file" name="thumbnail" id="thumbnail-input" accept="image/*" class="hidden" onchange="previewThumbnail(this)">
                        <label for="thumbnail-input" class="inline-flex items-center gap-2 px-4 py-2 rounded-lg border border-dashed border-[#D9C5A0]/40 text-[#D9C5A0] hover:border-[#D9C5A0] cursor-pointer transition-all text-sm font-semibold bg-surface-container-lowest/50">
