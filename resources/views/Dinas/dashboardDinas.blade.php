...
@section('title', 'SMWKP | Admin Dashboard')
...
<main class="ml-64 p-margin-desktop bg-background min-h-screen">
    <header class="flex justify-between items-end mb-12">
        <div>
            <h2 class="font-headline-lg text-headline-lg text-brand-beige mb-2">Ikhtisar Dashboard</h2>
            <p class="font-body-md text-body-md text-on-surface-variant max-w-xl">Selamat datang kembali. Pantau dan verifikasi data kuliner Kota Palembang secara real-time.</p>
        </div>
    ...
    <section class="grid grid-cols-1 gap-6 mb-12 lg:grid-cols-3">
        <div class="bg-surface-container-low p-6 rounded-xl card-subtle-border flex flex-col group hover:bg-surface-container transition-colors">
            <div class="flex justify-between items-start mb-4">
                <p class="font-label-md text-xs text-on-surface-variant uppercase tracking-widest">Total Listing</p>
                <span class="material-symbols-outlined text-brand-beige/20 group-hover:text-brand-beige/50 transition-colors">store</span>
            </div>
            ...
            <p class="text-[12px] text-secondary flex items-center gap-1 font-medium">
                ...
                Total pendaftaran terdaftar
            </p>
        </div>
        <div class="bg-surface-container-low p-6 rounded-xl card-subtle-border flex flex-col group hover:bg-surface-container transition-colors">
            <div class="flex justify-between items-start mb-4">
                <p class="font-label-md text-xs text-on-surface-variant uppercase tracking-widest">Menunggu Review</p>
                <span class="material-symbols-outlined text-error/20 group-hover:text-error/50 transition-colors">pending_actions</span>
            </div>
        ...
        <div class="bg-surface-container-low p-6 rounded-xl card-subtle-border flex flex-col group hover:bg-surface-container transition-colors">
            ...
            <h3 class="font-display-lg text-display-lg text-brand-beige mb-2">{{ $avgRating }}<span class="text-headline-md text-on-surface-variant font-normal">/5.0</span></h3>
            <p class="text-[12px] text-on-surface-variant font-medium">Dari {{ $reviewsCount }} ulasan</p>
        </div>
    </section>

    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <section class="lg:col-span-2 bg-surface-container-low rounded-xl card-subtle-border overflow-hidden">
            <div class="px-8 py-6 border-b border-outline-variant/10 flex justify-between items-center">
                <h3 class="font-headline-md text-headline-md text-brand-beige">Verifikasi Listing</h3>
                <a href="{{ route('dinas.verifikasi') }}" class="text-brand-beige text-xs font-label-md hover:underline">Lihat Semua</a>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-left">
                    <thead>
                        <tr class="border-b border-outline-variant/5">
                            <th class="px-8 py-4 font-label-md text-[11px] text-on-surface-variant uppercase tracking-widest">Informasi Tempat</th>
                            <th class="px-8 py-4 font-label-md text-[11px] text-on-surface-variant uppercase tracking-widest">Kategori &amp; Tanggal</th>
                    ...
                    <tbody class="divide-y divide-outline-variant/5">
                        ...
                        <tr class="hover:bg-white/[0.02] transition-colors group">
                            <td class="px-8 py-5">
                                <div class="flex items-center gap-4">
                                    <div class="w-12 h-12 rounded-lg overflow-hidden bg-surface-container-highest">
                                        <img alt="Food" class="w-full h-full object-cover grayscale-[0.2] group-hover:grayscale-0 transition-all" src="{{ $r->photo_url }}">
                                    </div>
                                    <div>
                            ...
                            <td class="px-8 py-5">
                                <div class="flex flex-col gap-1">
                                    <span class="text-[11px] text-secondary font-medium uppercase tracking-tight">{{ $r->kategori }}</span>
                                    <span class="text-[11px] text-on-surface-variant">Terdaftar: {{ $r->created_at->format('d M Y') }}</span>
                                </div>
                            </td>
                            <td class="px-8 py-5 text-right">
                                <div class="flex justify-end gap-3">
                                    <button class="p-2 rounded-lg bg-surface-container-high text-brand-beige hover:bg-brand-maroon hover:text-brand-beige transition-all btn-show-detail"
                        ...
                        @empty
                        <tr>
                            <td colspan="3" class="px-8 py-10 text-center text-on-surface-variant">Tidak ada pengajuan verifikasi dalam antrean.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <aside class="flex flex-col gap-6">
            <section class="bg-surface-container-low rounded-xl card-subtle-border p-6 flex flex-col gap-4">
                <div class="flex items-center gap-2 border-b border-outline-variant/5 pb-3">
                    <span class="material-symbols-outlined text-brand-beige text-[20px]">verified_user</span>
                    <h4 class="font-label-md text-[11px] text-brand-beige font-bold uppercase tracking-widest">Sertifikasi Halal</h4>
                </div>
                <div class="flex flex-col gap-1">
                    <div class="flex items-baseline justify-between">
                        <span class="text-display-lg text-brand-beige font-bold">85%</span>
                        <span class="text-[10px] text-on-surface-variant uppercase tracking-tighter">Kuartal Ini</span>
                    </div>
                    <div class="w-full bg-surface-container-highest h-1.5 rounded-full overflow-hidden">
                        <div class="bg-brand-maroon h-full w-[85%] rounded-full"></div>
                    </div>
                </div>
                <p class="text-[12px] text-on-surface-variant leading-relaxed">
                    Sistem audit berkas UMKM kuliner Palembang.
                </p>
            </section>
        </aside>
    </div>
