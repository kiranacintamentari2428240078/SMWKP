...
@section('title', 'Profil Dinas | SMWKP Dinas')
...
<!-- Main Content Canvas -->
<main class="ml-64 p-margin-desktop bg-background min-h-screen text-[#D9C5A0]">
    <div class="max-w-4xl mx-auto">
        
        <!-- Header -->
        <div class="flex items-center gap-4 mb-10">
            <a href="{{ route('dinas.dashboard') }}" class="text-on-surface-variant hover:text-brand-beige transition">
                <span class="material-symbols-outlined text-3xl">arrow_back</span>
            </a>
            <div>
                <h1 class="text-2xl font-bold text-white">Profil Admin Dinas</h1>
                <p class="text-on-surface-variant text-sm">Kelola informasi akun Anda</p>
            </div>
        </div>
        ...
        <form action="{{ route('dinas.profil.update') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
            @csrf
            ...
            <div class="bg-surface-container-low border border-outline-variant/10 rounded-2xl p-8 space-y-8">
                ...
                <div class="flex items-center gap-6">
                    ...
                    <div>
                        <p class="font-bold text-lg text-white">{{ Auth::user()->name ?? 'Admin Dinas' }}</p>
                        <p class="text-on-surface-variant text-sm">{{ Auth::user()->email ?? '-' }}</p>
                        <span class="inline-block mt-2 px-3 py-0.5 text-xs font-semibold rounded-full bg-[#5a0f16] text-[#f5f0e6] border border-primary/20">Admin Dinas</span>
                    </div>
                </div>
